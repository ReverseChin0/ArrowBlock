...
Shader "Shader Forge/UnlitArrow" {
    Properties {
        ...
        _RimColor ("RimColor", Color) = (0.9932423,1,0.6666667,1)
...
////// Emissive:
                ...
                float node_2576 = pow(1.0-max(0,dot(i.normalDir, viewDirection)),exp(_RimPower_var));
                ...
                float3 RimData = (node_2576*_RimIntensity_var*_RimColor_var.rgb);
                float3 node_5215 = RimData;
                float3 emissive = node_5215;
                ...
                float3 node_6645 = Background;
                float _Out_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Out );
                float3 finalColor = emissive + lerp(node_9624,node_6645,_Out_var);
...
////// Lighting:
                ...
                float3 node_6645 = Background;
                float _Out_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Out );
                float3 finalColor = lerp(node_9624,node_6645,_Out_var);
